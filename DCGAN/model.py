...
class Discriminator(nn.Module):
    def __init__(self, channels_img, features_d):
        ...
        # INPUT : N * channels_img x 64 x 64
...
class Generator(nn.Module):
    def __init__(self, z_dim, channels_img, features_g): 
        ...
        self.gen =  nn.Sequential(
                ...
                nn.Tanh() # since images are normalised from -1 to 1 so model output is also in -1 to 1
    ...
    def _block(self, in_channels, out_channels, kernel_size, stride, padding):
        return nn.Sequential(
                            nn.ConvTranspose2d(in_channels, 
                                                ...
                                                stride, 
                                                padding, 
                                                bias = False),#using bias = False because batchnorm is being used
                            nn.BatchNorm2d(out_channels),
                            nn.ReLU()
                            )
...
def test():
    ...
    print("Discriminator output Shape::",disc(x).shape)

    gen = Generator(z_dim, in_channels, 8)
    initialise_weight(gen)
    z = torch.randn(((N, z_dim, 1, 1)))
    assert(gen(z).shape == (N, in_channels, H, W))
    print("Generated Image Shape::",gen(z).shape)
